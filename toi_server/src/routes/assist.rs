...
use crate::models::{
    assist::{ChatResponseKind, HttpRequests, RequestResponse},
    client::ClientError,
    state::ToiState,
};
...
async fn chat(
    ...
    let chat_response_kind = chat_response_kind.parse::<u8>().map_err(|err| {
        ClientError::ResponseJson.into_response(
    ...
    let system_prompt = match chat_response_kind {
        ...
        | ChatResponseKind::AnswerWithHttpRequests => {
            ...
            let http_requests =
                serde_json::from_str::<HttpRequests>(&http_requests).map_err(|err| {
                    ClientError::ResponseJson.into_response(
                        &state.client.generation_api_config.base_url,
                        &err.to_string(),
                    )
            ...
            for http_request in http_requests.requests {
                ...
                let response = reqwest::Client::new()
                    ...
                    .await
                    .map_err(|err| {
                        ClientError::ApiConnection.into_response(
                            &state.client.generation_api_config.base_url,
                            &err.to_string(),
                        )
                    })?;
